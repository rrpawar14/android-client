...
class GroupsListFragment : MifosBaseFragment(), GroupsListMvpView, RecyclerItemClickListener.OnItemClickListener, OnRefreshListener, Filterable {
    ...
    private var mGroupList: MutableList<Group>? = null
    private var groupsFullList: List<Group>? = null
    ...
    override fun showGroups(groups: List<Group?>?) {
        mGroupList = groups as MutableList<Group>?
        mGroupList?.sortWith(Comparator { grp1, grp2 -> grp1.name.compareTo(grp2.name) })
        mGroupListAdapter!!.setGroups(groups)
        groupsFullList = ArrayList(mGroupList)
    }
     ...
     * @param groups
     */
    override fun showLoadMoreGroups(groups: List<Group?>?) {
        mGroupList?.addAll(groups as MutableList<Group>)
    ...
    private val filter: Filter = object : Filter() {
        override fun performFiltering(charSequence: CharSequence): FilterResults {
            val filteredList: MutableList<Group> = ArrayList()
            if (charSequence.toString().isEmpty()) {
                filteredList.addAll(groupsFullList!!)
            } else {
                val filterPattern = charSequence.toString().toLowerCase().trim { it <= ' ' }
                for (group in groupsFullList!!) {
                    if (group.name.toLowerCase().contains(filterPattern)) {
                        filteredList.add(group)
                    }
                    if (group.accountNo.contains(filterPattern)) {
                        filteredList.add(group)
                    }
                }
            }
        ...
        override fun publishResults(charSequence: CharSequence, filterResults: FilterResults) {
            mGroupList?.clear()
            mGroupList?.addAll(filterResults.values as MutableList<Group>)
            mGroupListAdapter!!.notifyDataSetChanged()
        }
    }
